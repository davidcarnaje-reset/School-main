...
async function flushDatabase() {
  ...
  try {
    ...
    for (const tableName of tableNames) {
      if (tableName === 'users') {
        // For users table, delete everything except the admin user (id = 1)
        console.log(`🧹 Cleaning users table: keeping only admin (id = 1)`);
        await connection.execute('DELETE FROM users WHERE id != 1');
        // Ensure school_id is 1 or whatever default is suitable (usually 1 or NULL)
        await connection.execute('UPDATE users SET school_id = 1 WHERE id = 1');
        // Let's reset the auto-increment value to 2
