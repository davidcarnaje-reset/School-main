...
async function getTables() {
  try {
    ...
    for (const name of tableNames) {
      output += `\n--- TABLE: ${name} ---\n`;
      const [columns] = await connection.execute(`DESCRIBE \`${name}\``);
      columns.forEach(col => {
        output += `  ${col.Field} - ${col.Type} - Null: ${col.Null} - Key: ${col.Key} - Default: ${col.Default}\n`;
      });
    }
    
    fs.writeFileSync(path.join(__dirname, 'tables-list.txt'), output);
    console.log('✅ Wrote tables-list.txt');
    await connection.end();
  } catch (error) {
    console.error('❌ Connection or schema query FAILED!');
